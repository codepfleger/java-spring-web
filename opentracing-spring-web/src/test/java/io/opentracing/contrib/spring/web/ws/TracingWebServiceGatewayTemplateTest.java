 ...
 */
public class TracingWebServiceGatewayTemplateTest {
    ...
    public TracingWebServiceGatewayTemplateTest() {
        Jaxb2Marshaller marshaller = new Jaxb2Marshaller();

        webServiceGatewaySupport = new WebServiceGatewaySupport() {
            @Override
            protected void initGateway() throws Exception {
                super.initGateway();
            }
        };
        webServiceGatewaySupport.setInterceptors(new ClientInterceptor[]{new TracingClientInterceptorAdapter(mockTracer)});
        webServiceGatewaySupport.setMarshaller(marshaller);
        webServiceGatewaySupport.setUnmarshaller(marshaller);

        mockServer = MockWebServiceServer.createServer(webServiceGatewaySupport);
    }
    ...
    public void testStandardTags() {
        ...
        webServiceGatewaySupport.getWebServiceTemplate().sendSourceAndReceive("/something", requestPayload, new SourceExtractor<Source>() {
            @Override
